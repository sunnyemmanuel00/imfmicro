...
class Login extends DBConnection {
    ...
    public function __construct(){
        ...
        try {
            ...
            if ($firebaseCredentials !== false && !empty($firebaseCredentials)) {
                // The credentials are provided as a JSON string.
                // We need to decode it into an associative array for the SDK.
                ...
                // Use withServiceAccount(), which accepts an array
                $factory = (new Factory)->withServiceAccount($serviceAccount);
                $this->firebaseAuth = $factory->createAuth();

            } else {
                // Fallback to a file path if the direct credentials are not set.
                // This might be for local development.
                $serviceAccountPath = getenv('FIREBASE_SERVICE_ACCOUNT_PATH');
                if ($serviceAccountPath !== false && file_exists($serviceAccountPath)) {
                    $factory = (new Factory)->withServiceAccount($serviceAccountPath);
                    $this->firebaseAuth = $factory->createAuth();
                } else {
                    // Final fallback for local dev if env var isn't set
                    $localJsonPath = __DIR__ . '/firebase-service-account.json';
                    if (file_exists($localJsonPath)) {
                        $factory = (new Factory)->withServiceAccount($localJsonPath);
                        $this->firebaseAuth = $factory->createAuth();
                    } else {
                        throw new Exception("Firebase service account configuration not found. Please ensure the FIREBASE_CREDENTIALS or FIREBASE_SERVICE_ACCOUNT_PATH environment variable is set, or the firebase-service-account.json file exists locally.");
                    }
                }
            }
        } catch (Throwable $e) {
    ...
    public function login(){
        ...
        try {
            if ($this->db_type === 'mysqli') {
                ...
                // For PostgreSQL with PDO, placeholders are typically question marks.
                $stmt = $this->conn->prepare("SELECT * FROM users WHERE username = ?");
    ...
    public function firebase_login_session() {
        ...
        try {
            ...
            if ($this->db_type === 'mysqli') {
                // SQL syntax for mysqli with backticks
                ...
                // SQL syntax for pgsql with double quotes
                $stmt = $this->conn->prepare('SELECT *, "status", "first_login_done", "transaction_pin" AS "pin" FROM "accounts" WHERE "firebase_uid" = ? OR lower(email) = ?');
