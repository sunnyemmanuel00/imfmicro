...
<script>
 ...
 $(function(){
  ...
  // ====================== START: THE FIX ======================
  // These are the missing event listeners that bind the button clicks to the functions.
  // We use event delegation with 'on' to ensure it works even if the content is loaded dynamically.
  $(document).on('click', '.edit_data', function(){
    var id = $(this).attr('data-id');
    // Call uni_modal to open the form in a modal
    uni_modal('Update Linked Account Details', '?page=linked_accounts/manage_linked_account&id=' + id, 'mid-large');
  });

  $(document).on('click', '.delete_data', function(){
    var id = $(this).attr('data-id');
    // Use the custom prompt modal to ask for the Transaction PIN
    uni_modal_prompt(
      'Confirm Deletion', 
      'Please enter your Transaction PIN to confirm the deletion of this linked account.',
      function(pin_input) {
        // This callback function is executed after the user enters the PIN and clicks 'Confirm'
        if(pin_input) {
          delete_linked_account(id, pin_input);
        } else {
          alert_toast("Transaction PIN cannot be empty.", 'error');
        }
      },
      'password' // Specify input type as password
    );
  });
  // ======================= END OF FIX ========================
});
